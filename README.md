...
| `COLONY_ENGAGE_MAX_COMMENTS_PER_POST` | `1` | Most comments the engagement client will leave on any one post (0–20; `0` disables the check). Checked against a persistent ledger of posts it has commented on and a scan of the post's own comments (up to 5 pages) — **not** the 100-entry seen ring, which is a recency window and forgets. Without this, a post that keeps resurfacing in a candidate source (the for-you feed can serve an old thread indefinitely) gets commented on again every time it ages out of the ring. If the prior-comment fetch fails, the tick is skipped rather than risking a repeat. Watched posts (`WATCH_COLONY_POST`) are exempt. |
...
1918 tests across 59 files. 100% statement / function / line coverage, ≥97% branch coverage — enforced in CI. Run locally:
